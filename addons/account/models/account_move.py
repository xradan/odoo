        ...
        for record in self:
            ...
            if record.state == 'draft' and not record.posted_before:
                if not record._get_previous_sequence('name'):
                    ...
                    record.name = '/'
            elif record.state == 'posted' and (record.name == '/' or not record.name):
                record._set_next_sequence('name')
        ...
        for move in self:
            ...
            try:
                with self.env.cr.savepoint():
                    self.env.cr.execute("SELECT {field} FROM res_partner WHERE ID=%s FOR UPDATE NOWAIT".format(field=field), (move.partner_id.id,))
                    self.env.cr.execute("UPDATE res_partner SET {field}={field}+1 WHERE ID=%s".format(field=field), (move.partner_id.id,))
